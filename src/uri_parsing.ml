...
module Parser = struct
  ...
  and Variant : sig
    ...
    val find_next_declared_path_pattern : 'a T.t -> Match_pattern.t option
  end = struct
    module type S = sig
      module Typed_variant : Typed_variants_lib.S
    ...
    let find_next_declared_path_pattern : type a. a T.t -> Match_pattern.t option =
      fun t ->
      let rec next_declared_path_pattern
        ...
        | New_parser { new_; prev; f = _ } ->
          let new_next_path = next_declared_path_pattern ~prefix new_
          and prev_next_path = next_declared_path_pattern ~prefix prev in
          let module T = struct
            type t =
              ([ `Ignore | `Match of string ] list
              * [ `Continue | `Stop_prefix | `Stop_remaining_path ])
             ...
             print_s
               [%message
                 "Ambiguity, a Parser.new_parser's new parser and previous parser have \
                  different expected paths. The previous parser won't ever parse."
                   (new_next_path : T.t)
                   (prev_next_path : T.t)]);
          new_next_path
      in
  ...
  let from_fragment ?(here = Stdlib.Lexing.dummy_pos) value_parser =
  ...
  let rec needs_to_appear_on_path_order : type a. a T.t -> bool = function
    ...
    | Query_based_variant { variant_module; _ } ->
      ...
      List.exists M.Typed_variant.Packed.all ~f:(fun { f = T v } ->
        needs_to_appear_on_path_order (M.parser_for_variant v))
    | New_parser { new_; prev; f = _ } ->
      needs_to_appear_on_path_order new_ || needs_to_appear_on_path_order prev
  ...
  module Eval = struct
    ...
    and eval_new_parser
      : type new_ prev.
        new_:(Components.t, new_ Parse_result.t) Projection.t
        -> prev:(Components.t, prev Parse_result.t) Projection.t
        -> f:(prev -> new_)
        -> (Components.t, new_ Parse_result.t) Projection.t
      =
      fun ~new_ ~prev ~f ->
      let parse_exn (components : Components.t) =
        try new_.parse_exn components with
        | _ ->
          let result = prev.parse_exn components in
          { Parse_result.result = f result.result; remaining = result.remaining }
      in
      let unparse (parse_result : new_ Parse_result.t) = new_.unparse parse_result in
      { Projection.parse_exn; unparse }
  ...
  module Skeleton = struct
    ...
    let rec of_parser : type a. a T.t -> t = function
      ...
      | New_parser { new_; prev; f = _ } ->
        let new_ = of_parser new_
        and prev = of_parser prev in
    ...
    let rec has_enough_information_on_its_own = function
      ...
      | New_parser { new_; prev } ->
        has_enough_information_on_its_own new_ && has_enough_information_on_its_own prev
      | Unit
      | From_path _
      ...
      | New_parser { new_; prev } ->
        ...
        and prev =
          all_url_shapes
            ...
            prev
        in
        (match [%equal: Url_shape.t list] new_ prev with
         | true -> new_
         | false -> new_ @ prev)
    in
    all_url_shapes
  ...
  let rec check_that_path_orders_are_ok : type a. a T.t -> unit = function
    ...
    | Query_based_variant { variant_module; _ } ->
      ...
      List.iter M.Typed_variant.Packed.all ~f:(fun { f = T v } ->
        check_that_path_orders_are_ok (M.parser_for_variant v))
    | New_parser { new_; prev; f = _ } ->
      check_that_path_orders_are_ok new_;
      check_that_path_orders_are_ok prev
  ;;
  ...
  let rec check_that_there_are_no_ambiguous_parsers_at_any_level : type a. a T.t -> unit =
    ...
    | New_parser { new_; prev; f = _ } ->
      check_that_there_are_no_ambiguous_parsers_at_any_level new_;
      check_that_there_are_no_ambiguous_parsers_at_any_level prev
  ;;

  let rec check_that_there_are_sane_new_parsers : type a. a T.t -> unit =
    fun t ->
    match t with
    | Unit -> ()
    ...
    | Record { record_module; override_namespace = _ } ->
      ...
      in
      List.iter M.Typed_field.Packed.all ~f:(fun { f = T f } ->
        check_that_there_are_sane_new_parsers (M.parser_for_field f))
    | Variant { variant_module; override_namespace = _ } ->
      let module M =
        (val variant_module : Variant.Cached_s with type Typed_variant.derived_on = a)
      in
      List.iter M.Typed_variant.Packed.all ~f:(fun { f = T v } ->
        check_that_there_are_sane_new_parsers (M.parser_for_variant v))
    | Query_based_variant { variant_module; override_namespace = _; key = _ } ->
      ...
      List.iter M.Typed_variant.Packed.all ~f:(fun { f = T v } ->
        check_that_there_are_sane_new_parsers (M.parser_for_variant v))
    | Optional_query_fields { t } -> check_that_there_are_sane_new_parsers t
    | New_parser { new_; prev; f = _ } ->
      check_that_there_are_sane_new_parsers new_;
      check_that_there_are_sane_new_parsers prev;
      let new_ = Variant.find_next_declared_path_pattern new_
      and prev = Variant.find_next_declared_path_pattern prev in
      (match [%equal: Variant.Match_pattern.t option] new_ prev with
       | true -> ()
       | false ->
         raise_s
           [%message
             "Ambiguity, a Parser.new_parser's new parser and previous parser have \
              different expected paths. The previous parser won't ever parse."
               (new_ : Variant.Match_pattern.t option)
  ...
  let both ?namespace (type first second) (first : first t) (second : second t)
    : (first * second) t
    =
    let module M = struct
      type 'a parser = 'a t
      ...
      let parser_for_field : type a. a Typed_field.t -> a parser = function
        | First -> first
        | Second -> second
      ;;
    ...
    in
    project
      (Record.make (module M) ?namespace)
      ~parse_exn:(fun { first; second } -> first, second)
      ~unparse:(fun (first, second) -> { first; second })
  ;;

  let new_parser new_ ~previous:prev ~f = T.New_parser { new_; prev; f }
end
